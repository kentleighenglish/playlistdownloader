var MainContainer = React.createClass({
	...
	getContentView: function(property) {
		...
		switch(property){
			case 'main':
				...
				break;
			case 'addFeed':
				content = <AddFeedView addFeed={this.addFeed} />;
				break;
			case 'configure':
				content = <ConfigureView updateConfiguration={this.updateConfiguration} />;
				break;
			case 'feed':
				content = <FeedView
	...
	getHeaderView: function(property) {
		...
		switch(property){
			case 'feed':
				header = <div>
					<a href="#" className="submenu__button" onClick={() => this.downloadList(false)}>Download All</a>
					<a href="#" className="submenu__button" onClick={() => this.downloadList(true)}>Download Selected</a>
				</div>;
				break;
			case 'main':
				header = <div>
					<a className="submenu__button" href="#" onClick={() => this.loadView("addFeed")}>Add Feed</a>
					<a className="submenu__button" href="#" onClick={() => feedUtility.updateFeeds() }>Update Feeds</a>
				</div>;
				break;
		}
		return header;
	...
	render: function() {
		...
		return (
			<div>
				<Header headerView={header} loadView={this.loadView} />
	...
	render: function() {
		return (
			<div className="container__header">
				<div className="menu">
					<a href="#" onClick={() => this.props.loadView('main')} className="menu__button">Manage Feeds</a>
					<a href="#" onClick={() => this.props.loadView('configure')} className="menu__button">Configuration</a>
				</div>
...
var FeedListItem = React.createClass({
	...
	render: function() {
		...
		return (
			<div className={'feedListItem '+newItem+(typeof(status) != null && status != undefined ? 'feedListItem--'+status : '')} >
				<div className="feedListItem__status"></div>
				<div className="feedListItem__progress" style={progressWidth} ></div>
				<div className="feedListItem__left">
					<span className="feedListItem__name">{itemData.name}{statusMessage}</span><br /><span className="feedListItem__date">{formattedDate}</span>
				</div>
				<div className="feedListItem__right">
					<span><a href="#" className="feedListItem__download" onClick={() => this.props.downloadItem(itemData.link, itemData.name, this.props.index)}>DL</a></span>
					{itemData.selected == true ?
						<span className="feedListItem__selectToggle feedListItem__selectToggle--checked" onClick={(e) => { var evt = new MouseEvent("click"); if(e.currentTarget == e.target){ document.querySelector('.checkbox label').dispatchEvent(evt); } }}>
							<label onClick={(e) => this.props.selectToggle(this.props.index)}>
								<input name="selectToggle" type="checkbox" value="selected" checked />
							</label>
						</span>
					:
						<span className="feedListItem__selectToggle" onClick={(e) => { var evt = new MouseEvent("click"); if(e.currentTarget == e.target){ document.querySelector('.checkbox label').dispatchEvent(evt); } }}>
							<label onClick={(e) => this.props.selectToggle(this.props.index)}>
								<input name="selectToggle" type="checkbox" value="selected" />
							</label>
						</span>
					}
				</div>
	...
	createFeedRows: function(feeds) {
		if(typeof(feeds) != 'undefined' && feeds != null){
			return feeds.map(function(feed, index){
				...
				return <tr className="feedsTable__row" onClick={() => this.props.loadFeed(index)}>
					<td className="feedsTable__bodyColumn">{feed.name}</td>
					<td className="feedsTable__bodyColumn">{feed.typeFormatted}</td>
					<td className="feedsTable__bodyColumn">{feed.newItems}</td>
					<td className="feedsTable__bodyColumn">{itemCount}</td>
					<td className="feedsTable__bodyColumn">{updated}</td>
					<td className="feedsTable__bodyColumn">buttons</td>
				</tr>;
			}, this);
		}else{
			return false;
		...
		return (
			<div>
				<table className="feedsTable">
					<thead>
						<tr>
							<th className="feedsTable__headColumn">Name</th>
							<th className="feedsTable__headColumn">Type</th>
							<th className="feedsTable__headColumn">New Items</th>
							<th className="feedsTable__headColumn">Items</th>
							<th className="feedsTable__headColumn">Last Updated</th>
							<th className="feedsTable__headColumn"></th>
						</tr>
					</thead>
					<tbody>
						{feedRows}
	...
	render: function() {
		...
		var feedTypeOptions = Object.keys(feedTypes).map(function(key){
			if (feedTypes.hasOwnProperty(key)) {
				return <option value={key}>{feedTypes[key]}</option>;
			}
		});

		return (
			<div>
				<form onSubmit={(e) => this.props.addFeed(e, this.state.formData)}>
					<input name="feed_name" type="text" value={formData["feed_name"]} onChange={this.handleChange} required />
					<select required name="feed_type" onChange={this.handleChange} value={formData['feed_type']} >
						{typeof(formData['feed_type']) == 'undefined' ?
							<option value selected disabled>Feed Type</option>
						:
							<option value disabled>Feed Type</option>
						}
						{feedTypeOptions}
					</select>
					<input name="feed_source" type="text" value={formData["feed_source"]} onChange={this.handleChange} required />
...
var ConfigureView = React.createClass({
	...
	render: function() {
		return (
			<div>
				<form onSubmit={(e) => this.props.updateConfiguration(e, this.state.formData)}>
